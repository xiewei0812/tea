	...
	<view class="page">
		...
		<view class="login-box">
			<view class="login-card-title">
				<view class="login-card-title-content"> 账号注册 </view>
				<view class="login-card-title-content"></view>
			</view>
			<view class="login-card-input">
				<!-- 输入框头部图标 -->
				<uni-easyinput class="hj-input" type="text" v-model="regForm.username" focus maxlength="30" :trim="true"
					@focus="UserNamePassWrodFocusBorderStyle" :styles="inputColorConfigToUserName"
					@blur="UserNamePassWrodBlurBorderStyle" placeholder="请输入用户名"></uni-easyinput>
				<!-- 输入框头部图标 -->
				<uni-easyinput class="hj-input" v-model="regForm.password" type="password"
					@focus="PassWordFocusBorderStyle" @blur="PassWordBlurBorderStyle"
					:styles="inputColorConfigToPassWord" placeholder="请输入密码"></uni-easyinput>
			</view>
			<view class="login-card-loginIn">
				<view class="login-card-loginIn-btn" @click="regbtn">
					注册
				</view>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			...
			async regbtn() {
				if (this.regForm.username.length >= 7) {
				...
				if (this.regForm.password.length >= 7) {
					return uni.showToast({
						title: '密码过长',
						icon: 'error'
					})
